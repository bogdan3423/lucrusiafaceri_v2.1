...
import { MapPin, Clock, Share2, ImageOff, ChevronLeft, ChevronRight, X, Play, Heart, MessageCircle, Send, Trash2 } from 'lucide-react';
...
const PostCard = memo(function PostCard({ post, priority = false }: PostCardProps) {
  ...
  const shareToApp = useCallback((platform: string) => {
    const postUrl = `${window.location.origin}/postare/${post.id}`;
    const text = `${post.title}${post.price ? ` - ${formatPrice(post.price, post.currency)}` : ''}`;
    ...
    switch (platform) {
      ...
      case 'facebook':
        ...
        break;
      case 'instagram':
        // Instagram doesn't have a direct share URL, copy link and show instructions
  ...
  }, [post.id, post.title, post.price, post.currency]);
            ...
            <div className="relative">
              ...
              {showShareMenu && (
                <>
                  ...
                  {/* Share Panel */}
                  <div className="fixed inset-x-0 bottom-0 bg-white rounded-t-2xl p-4 pb-8 z-50 md:absolute md:inset-auto md:bottom-full md:right-0 md:mb-2 md:rounded-xl md:shadow-xl md:border md:border-gray-100 md:p-2 md:min-w-[200px]">
                    {/* Mobile Header */}
                    <div className="flex items-center justify-between mb-4 md:hidden">
                      <h3 className="text-lg font-semibold text-gray-900">Distribuie</h3>
                      <button 
                        onClick={() => setShowShareMenu(false)}
                    ...
                    {/* Share Options - Grid on mobile, List on desktop */}
                    <div className="grid grid-cols-4 gap-4 md:flex md:flex-col md:gap-0">
                      {/* WhatsApp */}
                      <button
                        onClick={() => shareToApp('whatsapp')}
                        className="flex flex-col items-center gap-2 md:flex-row md:gap-3 md:w-full md:px-3 md:py-2.5 md:hover:bg-gray-50 md:rounded-lg transition-colors"
                      >
                        <div className="w-14 h-14 md:w-9 md:h-9 rounded-full bg-green-500 flex items-center justify-center shadow-sm">
                          <svg className="w-7 h-7 md:w-5 md:h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <span className="text-xs md:text-sm font-medium text-gray-700">WhatsApp</span>
                      </button>
                      ...
                      <button
                        ...
                        className="flex flex-col items-center gap-2 md:flex-row md:gap-3 md:w-full md:px-3 md:py-2.5 md:hover:bg-gray-50 md:rounded-lg transition-colors"
                      >
                        <div className="w-14 h-14 md:w-9 md:h-9 rounded-full bg-blue-600 flex items-center justify-center shadow-sm">
                          <svg className="w-7 h-7 md:w-5 md:h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                          </svg>
                        </div>
                        <span className="text-xs md:text-sm font-medium text-gray-700">Facebook</span>
                      </button>
                      ...
                      <button
                        ...
                        className="flex flex-col items-center gap-2 md:flex-row md:gap-3 md:w-full md:px-3 md:py-2.5 md:hover:bg-gray-50 md:rounded-lg transition-colors"
                      >
                        <div className="w-14 h-14 md:w-9 md:h-9 rounded-full bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 flex items-center justify-center shadow-sm">
                          <svg className="w-7 h-7 md:w-5 md:h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <span className="text-xs md:text-sm font-medium text-gray-700">Instagram</span>
                      </button>
                      ...
                      <button
                        ...
                        className="flex flex-col items-center gap-2 md:flex-row md:gap-3 md:w-full md:px-3 md:py-2.5 md:hover:bg-gray-50 md:rounded-lg transition-colors"
                      >
                        <div className="w-14 h-14 md:w-9 md:h-9 rounded-full bg-gray-200 flex items-center justify-center shadow-sm">
                          <svg className="w-6 h-6 md:w-4 md:h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                          </svg>
                        </div>
                        <span className="text-xs md:text-sm font-medium text-gray-700">Copiază</span>
                      </button>
                    </div>
                  </div>
                </>
